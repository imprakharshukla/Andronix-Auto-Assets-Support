...
/support/common/busybox_static tar -xzvf /support/rootfs.tar.gz --exclude sys --exclude dev --exclude proc --exclude data --exclude mnt --exclude host-rootfs --exclude support --exclude sdcard --exclude etc/mtab --exclude usr/local/bin/sudo --exclude etc/profile.d/andronix_profile.sh --exclude etc/ld.so.preload -C /
